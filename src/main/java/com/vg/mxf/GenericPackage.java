    ...
    boolean handleTag(int localTag, int sz, ByteBuffer buf) {
        if (!super.handleTag(localTag, sz, buf)) {
            if (Arrays.binarySearch(localTags, localTag) >= 0) {
                switch (localTag) {
                case 0x4401:
                    PackageUID = inner(new TagUMID());
                    break;
                case 0x4402:
                    Name = inner(new TagUTF16String(sz));
                    break;
                case 0x4405:
                    PackageCreationDate = inner(new TagDate());
                    break;
                case 0x4404:
                    PackageModifiedDate = inner(new TagDate());
                    break;
                case 0x4403:
                    int elementCount = buf.getInt();
                    Tracks = inner(new TagUUIDList(elementCount));
                    break;
                }
                return true;
            }
